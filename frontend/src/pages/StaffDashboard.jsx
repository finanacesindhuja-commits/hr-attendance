            ...
            <div className="relative mt-4 flex flex-col items-center">
              ...
              <div className="w-full space-y-4">
                ...
                <div className="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-100/50 mt-4">
                  <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-3">Bank Passbook</p>
                  {staff.passbook_url ? (
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-green-600 flex items-center gap-1">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                      ...
                      <a href={staff.passbook_url} target="_blank" rel="noreferrer" className="text-[10px] font-black text-indigo-600 uppercase underline">View Document</a>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center py-2">
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          const formData = new FormData();
                          formData.append('passbookFile', file);
                          formData.append('staff_id', staff.staff_id);

                          try {
                            setActionLoading(true);
                            const res = await axios.post(`${API_URL}/staff/upload-passbook`, formData);
                            // Update local staff state
                            const updatedStaff = { ...staff, passbook_url: res.data.url };
                            setStaff(updatedStaff);
                            localStorage.setItem('staffInfo', JSON.stringify(updatedStaff));
                            alert('Uploaded Successfully!');
                          } catch (err) {
